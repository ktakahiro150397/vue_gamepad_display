...
class ImageSelector {
  ...
  constructor (el_parent) {
    ...
    const els_selected = el_parent.getElementsByClassName(
      'ImageSelector__s-selected'
    )
    if (els_selected.length === 0)
      this.#els_option[0].classList.add('ImageSelector__s-selected')
    const el_clone_option_first =
      els_selected.length === 0
        ? this.#els_option[0].cloneNode(true)
        : els_selected[0].cloneNode(true)
    ...
    //外部クリック処理
    const css_celector =
      el_parent.id === '' ? '.ImageSelector' : '#' + el_parent.id
